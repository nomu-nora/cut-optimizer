# アルゴリズム詳細仕様書

**プロジェクト**: 取り数最適化システム v1.0
**作成日**: 2025年12月2日
**最終更新**: 2025年12月2日

---

## 📌 このドキュメントについて

このドキュメントは、実装時に判断が必要な細かい仕様を記録したものです。
要件定義書やアーキテクチャ設計書で不明確だった点を、Q&Aを通じて明確にした内容をまとめています。

---

## 1. 余白とカット幅の扱い

### 1.1 余白の適用範囲

**仕様**: 元板の**上下左右すべての辺**に余白を設ける

```
┌─────────────────────────────────┐
│ ← 余白 20mm                     │
│  ↑                              │
│  余白 20mm                       │
│  ┌─────────────────────────┐   │
│  │                         │   │
│  │   有効エリア             │   │
│  │                         │   │
│  └─────────────────────────┘   │
│                  余白 20mm ↓   │
│                     余白 20mm → │
└─────────────────────────────────┘

有効幅 = 元板幅 - (余白 × 2)
有効高さ = 元板高さ - (余白 × 2)

例: 1820×910mmの元板で余白20mmの場合
有効エリア = 1780×870mm
```

**理由**: 実家の業務では、元板の端が傷ついている可能性があるため、全辺に保険スペースが必要。

---

### 1.2 カット幅の適用範囲

**仕様**: カット幅は**製品と製品の間のみ**に適用

```
余白で端はカバーされるため、以下には適用しない:
- 元板の端（余白の内側）と最初の製品の間
- 最後の製品の後ろ（右側・下側）

┌─────────────────────────────────┐
│ [余白]                          │
│  ┌────┐ ←カット幅→ ┌────┐      │
│  │ A  │    4mm     │ B  │      │
│  └────┘            └────┘      │
│  ↑                              │
│  カット幅なし（余白で十分）      │
└─────────────────────────────────┘
```

**理由**: 余白が既に確保されているため、端に追加のカット幅は不要。

---

## 2. 製品のソート順序

### 2.1 基本ソート: 面積の大きい順

**仕様**: 製品を面積順（大→小）にソート

```typescript
items.sort((a, b) => {
  const areaA = a.width * a.height
  const areaB = b.width * b.height
  return areaB - areaA // 降順
})
```

---

### 2.2 同じ面積の場合: アスペクト比順

**仕様**: 面積が同じ製品は、**アスペクト比が1:1に近い順**にソート

```typescript
// アスペクト比の計算
function getAspectRatio(item: Item): number {
  const ratio = item.width / item.height
  // 1に近いほど正方形
  return Math.abs(ratio - 1)
}

items.sort((a, b) => {
  const areaA = a.width * a.height
  const areaB = b.width * b.height

  if (areaA === areaB) {
    // 面積が同じ場合、アスペクト比が1に近い順
    return getAspectRatio(a) - getAspectRatio(b)
  }

  return areaB - areaA
})
```

**例**:

```
製品A: 400×400 (面積 160,000) → アスペクト比 1.0
製品B: 500×320 (面積 160,000) → アスペクト比 1.56
製品C: 800×200 (面積 160,000) → アスペクト比 4.0

ソート結果: A → B → C
```

**理由**: 正方形に近い製品の方が、配置時に柔軟性が高く、スペースを効率的に使える傾向がある。

---

## 3. 回転判定のロジック（重要）

### 3.1 問題の定義

製品を配置する際、90度回転させるかどうかの判断が難しい：

```
問題:
- Bを回転なしで配置（その時点では歩留まり良）
  → でも次のCが入らない → 結果的に歩留まり悪

- Bを回転ありで配置（その時点では歩留まり悪）
  → 次のCも入る → 結果的に歩留まり良
```

これは**「局所最適 ≠ 全体最適」**の典型的な問題。

---

### 3.2 v1.0の解決策: 残スペース最大化ヒューリスティック

**仕様**: 回転なし/ありの両方を試して、**配置後の最大空きスペースが大きくなる方**を選ぶ

```typescript
function decideRotation(item: Item, space: FreeSpace, cutConfig: CutConfig): boolean {
  // 1. 回転なしで配置を試す
  if (canPlace(item, space, cutConfig, false)) {
    const spacesWithoutRotation = simulatePlacement(item, space, cutConfig, false)
    const maxSpaceWithoutRotation = getMaxSpace(spacesWithoutRotation)

    // 2. 回転ありで配置を試す
    if (canPlace(item, space, cutConfig, true)) {
      const spacesWithRotation = simulatePlacement(item, space, cutConfig, true)
      const maxSpaceWithRotation = getMaxSpace(spacesWithRotation)

      // 3. 最大空きスペースが大きい方を選ぶ
      if (maxSpaceWithRotation.area > maxSpaceWithoutRotation.area) {
        return true // 回転あり
      }
    }
  }

  return false // 回転なし（または回転ありのみ可能な場合は別処理）
}

// 最大の空きスペースを取得
function getMaxSpace(spaces: FreeSpace[]): FreeSpace {
  return spaces.reduce((max, space) => {
    const area = space.width * space.height
    const maxArea = max.width * max.height
    return area > maxArea ? space : max
  })
}
```

---

### 3.3 判定フロー

```
製品Bを配置する場合:

1. 回転なしで配置シミュレーション
   → 残った最大スペース: 500×300 (150,000mm²)

2. 回転ありで配置シミュレーション
   → 残った最大スペース: 600×400 (240,000mm²)

3. 比較
   240,000 > 150,000
   → 回転ありを採用

理由: 次の製品が入りやすくなる
```

---

### 3.4 タイブレーク（同じ場合）

最大スペースが同じ場合は、**回転なしを優先**（シンプルな方を選ぶ）

```typescript
if (maxSpaceWithRotation.area > maxSpaceWithoutRotation.area) {
  return true // 回転あり
} else {
  return false // 同じまたは回転なしが良い場合
}
```

---

### 3.5 将来の改善案（v2.0以降）

より高度なアルゴリズムの候補：

1. **Look-ahead（先読み）**: 次のN個の製品も考慮して判断
2. **Branch and Bound**: 複数パターンを試して最適解を探索
3. **遺伝的アルゴリズム**: 複数の配置パターンを進化させる
4. **機械学習**: 過去のデータから最適な判断を学習

v1.0では計算速度とシンプルさを優先し、ヒューリスティックを採用。

---

## 4. 元板より大きい製品の扱い

### 4.1 現状の業務フロー

**実家の業務**:

- たまに元板より大きい製品の受注がある
- 元板サイズはメーカーの固定サイズで、それより大きいものは用意されない
- その場合、製品を分割裁断して複数の元板から切り取り、後でつなぐ

---

### 4.2 v1.0の対応方針

**仕様**: 警告を出してスキップ（計算から除外）

```typescript
function validateItems(
  items: Item[],
  plateConfig: PlateConfig,
  cutConfig: CutConfig
): ValidationResult {
  const errors: ValidationError[] = []
  const warnings: ValidationWarning[] = []
  const validItems: Item[] = []
  const skippedItems: Item[] = []

  // 有効エリアの計算
  const effectiveWidth = plateConfig.width - cutConfig.margin * 2
  const effectiveHeight = plateConfig.height - cutConfig.margin * 2

  for (const item of items) {
    // 元板より大きい製品をチェック
    if (item.width > effectiveWidth || item.height > effectiveHeight) {
      // 回転しても入らない場合
      if (item.height > effectiveWidth || item.width > effectiveHeight) {
        warnings.push({
          type: 'ITEM_TOO_LARGE',
          itemId: item.id,
          message: `製品「${item.name}」は元板より大きいため計算できません。この製品をスキップします。`,
        })
        skippedItems.push(item)
        continue
      }
    }

    validItems.push(item)
  }

  return { validItems, skippedItems, errors, warnings }
}
```

---

### 4.3 UI表示

```
計算実行時に警告を表示:

┌─────────────────────────────────────────┐
│ ⚠ 警告                                  │
│                                         │
│ 以下の製品は元板より大きいため          │
│ 計算から除外されました:                 │
│                                         │
│ • 製品X (2000×1000mm)                   │
│ • 製品Y (1900×950mm)                    │
│                                         │
│ これらの製品を含めて計算する場合は、    │
│ 元板サイズを変更するか、製品サイズを    │
│ 調整してください。                      │
│                                         │
│    [了解]  [計算をキャンセル]           │
└─────────────────────────────────────────┘

計算結果の下部に表示:

┌─────────────────────────────────────────┐
│ 計算されなかった製品 (2件)              │
├─────────────────────────────────────────┤
│ • 製品X (2000×1000mm) - 元板より大きい  │
│ • 製品Y (1900×950mm) - 元板より大きい   │
└─────────────────────────────────────────┘
```

---

### 4.4 将来の拡張（v3.0以降）

**分割裁断機能**の実装:

```
機能:
1. 製品を複数の矩形に自動分割
2. 各パーツを元板に配置
3. 結果に「組み立て指示」を含める

例:
製品X (2000×1000mm) を分割
→ パーツ1 (1000×1000mm) + パーツ2 (1000×1000mm)
→ 結合線を表示
```

---

## 5. パターンの表示順序

### 5.1 仕様

**パターンを枚数が多い順に表示**

```typescript
patterns.sort((a, b) => b.count - a.count)

// パターンIDを割り当て
patterns.forEach((pattern, index) => {
  pattern.patternId = String.fromCharCode(65 + index) // A, B, C...
})
```

---

### 5.2 表示例

```
計算結果:

┌───────────────────┐  ┌───────────────────┐
│ パターンA ×8枚    │  │ パターンB ×5枚    │
│ 歩留まり: 87.5%   │  │ 歩留まり: 85.2%   │
└───────────────────┘  └───────────────────┘

┌───────────────────┐  ┌───────────────────┐
│ パターンC ×3枚    │  │ パターンD ×1枚    │
│ 歩留まり: 90.1%   │  │ 歩留まり: 88.3%   │
└───────────────────┘  └───────────────────┘
```

**理由**:

- よく使うパターン（枚数が多い）を最初に表示
- 実際の作業では、大きいものから切り出すため、枚数が多いパターンを先に確認したい

---

## 6. 色の割り当て

### 6.1 仕様

**v1.0では自動割り当てのみ**

- 同じ製品名の場合、同じ色を使用
- 新しい製品の場合、既に使われていない色からランダムに選択
- 手動変更機能は実装しない（v1.5以降で検討）

```typescript
function assignColor(itemName: string, existingItems: Item[]): string {
  // 既存の製品と同じ名前があれば、同じ色を使う
  const existingItem = existingItems.find((i) => i.name === itemName)
  if (existingItem) {
    return existingItem.color
  }

  // 新しい製品の場合、未使用の色を選ぶ
  const usedColors = new Set(existingItems.map((i) => i.color))
  const availableColors = COLOR_PALETTE.filter((c) => !usedColors.has(c))

  if (availableColors.length > 0) {
    return availableColors[Math.floor(Math.random() * availableColors.length)]
  }

  // すべての色が使われている場合、パレットから選ぶ
  return COLOR_PALETTE[Math.floor(Math.random() * COLOR_PALETTE.length)]
}

const COLOR_PALETTE = [
  '#FF6B6B',
  '#4ECDC4',
  '#45B7D1',
  '#FFA07A',
  '#98D8C8',
  '#F7DC6F',
  '#BB8FCE',
  '#85C1E2',
  '#F8B739',
  '#52B788',
  '#E76F51',
  '#2A9D8F',
]
```

---

## 7. 想定規模とパフォーマンス

### 7.1 実家の業務規模

**想定入力**:

- 製品種類: 約10種類
- 総個数: 30〜50個
- 元板サイズ: 1820×910mm（サブロク板）
- 計算頻度: 1日2回程度

---

### 7.2 パフォーマンス目標

| 項目     | 目標           |
| -------- | -------------- |
| 計算時間 | 3秒以内        |
| UI応答性 | 即座（<100ms） |
| 描画速度 | 1秒以内        |

---

### 7.3 スケーラビリティ

v1.0での上限（目安）:

- 製品種類: 20種類まで
- 総個数: 100個まで
- 元板枚数: 30枚まで

これを超える場合は、v2.0でアルゴリズム最適化を検討。

---

## 8. テスト用プリセット

### 8.1 仕様

**1パターンの汎用的なテストデータを用意**

```json
{
  "plateConfig": {
    "width": 1820,
    "height": 910,
    "unitPrice": 2000
  },
  "cutConfig": {
    "cutWidth": 4,
    "margin": 20
  },
  "items": [
    {
      "id": "preset-1",
      "name": "大型パネル",
      "width": 600,
      "height": 400,
      "quantity": 5,
      "color": "#FF6B6B"
    },
    {
      "id": "preset-2",
      "name": "中型パネル",
      "width": 400,
      "height": 300,
      "quantity": 8,
      "color": "#4ECDC4"
    },
    {
      "id": "preset-3",
      "name": "小型パネル",
      "width": 200,
      "height": 150,
      "quantity": 12,
      "color": "#45B7D1"
    }
  ]
}
```

**配置場所**: `public/presets/test-data.json` または `src/lib/presets.ts`

---

### 8.2 プリセットの読み込み

```typescript
// UIに「テストデータを読み込む」ボタンを追加
function loadPreset() {
  const preset = TEST_PRESET
  setPlateConfig(preset.plateConfig)
  setCutConfig(preset.cutConfig)
  setItems(preset.items)
}
```

v1.0デプロイ後にテストして、問題なければプリセットを削除または非表示化。

---

## 9. 印刷物に含める情報

### 9.1 必須情報

**v1.0で印刷物に含める情報**:

1. **ビジュアル図** (最重要)
   - 元板の配置図（色分け、製品名表示）

2. **各元板の製品数**
   - 1枚の元板から各製品が何個切り出せているか
   - 例: アイテムA×3, アイテムB×2

3. **合計製品数**
   - すべての元板から合計で何個切り出せているか
   - 例: アイテムA 合計15個（要求15個）

4. **元板の必要枚数**
   - 例: 合計12枚必要

5. **歩留まり率**
   - 各パターンの歩留まり率
   - 全体の平均歩留まり率

---

### 9.2 将来追加する情報（v1.5以降）

- 計算日時
- 作業者名
- 注文番号
- 備考欄

---

## 10. エッジケースとエラーハンドリング

### 10.1 製品が1つも配置できない場合

```typescript
// すべての製品が配置できなかった場合
if (plates.length === 0 || plates.every((p) => p.placements.length === 0)) {
  throw new Error(
    '製品を配置できませんでした。元板サイズを大きくするか、製品サイズを小さくしてください。'
  )
}
```

---

### 10.2 製品数が一致しない場合

計算後、要求個数と配置個数を検証:

```typescript
function validateResult(items: Item[], result: CalculationResult): void {
  const placedCounts = new Map<string, number>()

  // 配置された個数をカウント
  result.patterns.forEach((pattern) => {
    pattern.placements.forEach((placement) => {
      const count = placedCounts.get(placement.item.id) || 0
      placedCounts.set(placement.item.id, count + 1)
    })
  })

  // 要求個数と比較
  items.forEach((item) => {
    const placedCount = (placedCounts.get(item.id) || 0) * pattern.count
    if (placedCount < item.quantity) {
      console.warn(
        `製品「${item.name}」の配置個数が不足しています。` +
          `要求: ${item.quantity}個, 配置: ${placedCount}個`
      )
    }
  })
}
```

---

## 11. 更新履歴

| 日付       | 更新内容                                    |
| ---------- | ------------------------------------------- |
| 2025-12-02 | 初版作成。Q&Aで明確になった仕様をすべて記録 |

---

## 12. 参考資料

- [要件定義書](./torikazu_app_requirements_v1.0.md)
- [アーキテクチャ設計書](./architecture.md)
- [データフロー設計書](./data-flow.md)

---

**このドキュメントは実装時の判断基準として使用してください。**
**不明点があれば、このドキュメントを更新して記録を残してください。**
