...
- ✅ **データベース統合** 🆕
  - Supabase（PostgreSQL）による完全な永続化
  - ユーザー認証（メール/パスワード + Google OAuth）
  - ユーザーごとのデータ完全分離（Row Level Security）
- ✅ **設定画面** 🆕
  - デフォルト値の一元管理
  - 最適化設定の保存
  - アカウント管理
...
- **データベース**: [Supabase](https://supabase.com/) (PostgreSQL + Auth)
...
**データベース統合 - 完全な永続化とユーザー管理** ✅

**実装機能**:

- **Supabase統合**
  - PostgreSQLデータベースによる完全な永続化
  - Row Level Security（RLS）によるユーザーデータの完全分離
  - リアルタイムデータ同期対応
- **認証システム**
  - メール/パスワード認証
  - Google OAuth対応
...
- **UX改善**
  ...
  - Enterキーでフォーム間移動（設定、テンプレート保存）
  - 元板単価 → カット幅へスムーズに移動
  - ダブルクリック編集による誤操作防止

**データベーススキーマ**:

- `user_settings`: ユーザーごとのデフォルト設定
- `product_templates`: 製品テンプレート
- `offcut_templates`: 端材テンプレート
- `calculation_history`: 計算履歴（is_starred列付き）

**セキュリティ**:

- RLS（Row Level Security）による完全なユーザーデータ分離
- HTTPSのみ（Vercel自動設定）
- 環境変数による認証情報管理
- public APIキーのみ使用（service_roleキーは不使用）

**改善**:

- データの永続化により、ブラウザを閉じても設定・履歴が保持
- 複数デバイス間でのデータ同期
- よく使う設定をテンプレートとして再利用可能
- 過去の計算結果を簡単に参照・復元
- ユーザーごとに完全に独立したデータ管理
