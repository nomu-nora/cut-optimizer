...
**現在のバージョン**: v1.6 ✅ (データベース統合完了・本番デプロイ済み)
**次のバージョン**: v1.7 🎯 (今後の機能拡張 - 検討中)
...
- ✅ **行クリックで編集モード開始**
  - 製品・端材一覧で行全体をクリックして編集開始
  - 編集ボタンを廃止してより直感的に
- ✅ **Enterキーでフィールド移動**
  - Tabキーに加えてEnterキーでも次のフィールドへ移動
  - 最後のフィールドでEnterを押すと保存/追加を実行
- ✅ **編集開始時の自動フォーカス**
  - 編集モードに入ると自動的に最初のフィールドにフォーカス
  - useRefとonKeyDownハンドラーで実装
...
- ✅ 厚み対応機能（複数厚みの製品・元板対応）
- ✅ カット工程効率化（カット順序最適化、作業時間推定）

#### ✅ v1.4.3 - マルチ選択とキーボード操作（完了）
...
- ✅ **マルチ選択機能**
  ...
  - Escapeキーで選択解除
- ✅ **キーボード位置調整**
  - 矢印キー: 選択された製品を1mmずつ移動
  - Shift+矢印キー: 10mmずつ移動
  - 複数選択時は全ての製品が同時に移動
  - リアルタイムで歩留まりと配置の妥当性を更新

**実装詳細**:

- `selectedPlacements: Set<string>` でマルチ選択状態を管理
- `getPlacementId()` で placement の一意なID生成
- EditablePlacementDiagram に keyboard event listener 追加
- DraggableProduct で Ctrl/Shift+Click を検出して選択モードに

**成果**:

- 複数の製品を同時に選択して移動可能
- 精密な位置調整がキーボードで可能に
- 既存の操作（クリックで仮置き場移動、ダブルクリックで回転）を維持
...
### ✅ v1.6 - データベース統合・ユーザー管理（完了）

**リリース日**: 2025年12月26日

**目的**:
Supabaseを導入し、完全な永続化・ユーザー管理・履歴管理を実現する。
...
- ✅ ユーザー認証・ログイン機能（メール/パスワード + Google OAuth）
- ✅ データモデル設計と実装
