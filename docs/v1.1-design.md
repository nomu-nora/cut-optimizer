...
function mutation(individual: Individual, mutationRate: number = 0.1): Individual {
  ...
  if (Math.random() < mutationRate) {
    // 回転戦略をランダムに変更
    const rotationStrategies: RotationStrategy[] = ['max-space', 'fit-space', 'prefer-rotate', 'no-rotate']
    individual.rotationStrategy = rotationStrategies[Math.floor(Math.random() * rotationStrategies.length)]
  }
...
<div className="calculating">
  <Spinner />
  <p>最適な配置を探索中... (世代 {currentGen}/{totalGen})</p>
