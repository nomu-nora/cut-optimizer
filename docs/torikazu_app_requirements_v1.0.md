...
3. 製品の配置ループ
   for each 製品 in ソート済み製品リスト:
       3.1 現在の元板に配置を試みる
           - 90度回転なしで配置可能？
           - 90度回転ありで配置可能？
           - 残スペース最大化ヒューリスティックで判定
             (配置後の最大空きスペースが大きくなる方を選択)

       3.2 配置できたら
           - 製品を配置
           - カット幅を考慮（製品間のみ、端は余白でカバー）
           - 残りスペースを2分割
               - 横分割（右側に残る）
               - 縦分割（下側に残る）
           - 残りスペースをリストに追加
...
## 13. 補足・詳細仕様（2025年12月2日更新）

### 13.1 実装詳細の補足

以下の内容は、実装時のQ&Aを通じて明確にした詳細仕様です。
...
- [UI/UX設計書](./ui-ux-design.md) - UIの詳細設計
